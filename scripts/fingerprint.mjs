...
export function differences(kind, then, now) {
  const u = usesFor(kind);
  const out = [];
  const say = { snippet: 'the snippet (html, css, js as the site loads them)', play: 'how it is played (the ways interaction.ts gives it)', text: 'its text (title, description, how, technique, tags, category)' };
...
export const STALENESS_TEXT = 'A result or review is stale when something it judged has changed since, and only then. '
  ...
  + 'the export check: the same, its render path adding the capture and recording code; the share-preview check: the same plus the text (the title is drawn in the image); '
...
const WORD = { snippet: 'render changed', play: 'how it is played changed (interaction.ts)', text: 'text changed' };
/**
