...
buttons.forEach((btn) => {
  ...
  timeline: {
    how: [
      ...
      'Each card works out its own <code>--d = --i − --a</code>. Opacity is <code>clamp(0, 1 + d·4, 1)</code>: 1 at the front, 0 as soon as a card is passed. Distance is shown with <code>filter: brightness(clamp(0.8, 1 − d·0.1, 1))</code> instead of a fade, and never goes under 0.8, so the words on a far card still read. Passed cards also get <code>min(d + 1, 0)</code> extra steps back, so nothing ever gets closer than one step to the camera.',
