...
export const snippet: Snippet = {
  how: [
    ...
    'The colour is <code>color-mix()</code>: teal for a gain, pink for a loss (<code>.down</code> swaps it), mixed with grey by <code>30% + --v × 70%</code>, so small moves are dull and big ones glow. The walls are glass, the colour mixed with <code>transparent</code> (about 60%); the roof is that colour over the chart’s own dark instead, because see-through it went pale under its own light words on a light stage. Both get a 0.6-unit bright edge and a soft inner glow.',
