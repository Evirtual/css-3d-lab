...
- [ ] Every model draws the same whatever box-sizing the page around it sets — `npm run capture -- boxsizing` ends with `135/135 models do not depend on outside CSS.` and exits 0; any `content-box by design` line it prints names a reason someone has read
- [ ] Every model's text is readable on the dark stage and the light one — `npm run capture -- contrast` ends with `135/135 models have readable text on both stages.` and exits 0
...
- [ ] No check result behind the ledger was judged under an older rule — `node -p "require('./docs/ledger.json').models.flatMap(m=>Object.values(m.checks)).filter(c=>(c.staleWhy||[]).some(w=>w.startsWith('rule changed'))).length"` prints `0`
...
- [ ] VIEW-CONTRACT.md's numbers are the checks' numbers — the table under "What must hold" matches the constants at the top of scripts/check-models.mjs (`grep -n "^const [A-Z_]* = " scripts/check-models.mjs`)
