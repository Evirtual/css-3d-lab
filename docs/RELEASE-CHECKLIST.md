...
- [x] The build is clean — `npm run build` exits 0 and prints `generated 135 model pages, 8 group pages` — run 2026-09-25 at `3c4100b`: exits 0 in 20s and prints `generated 135 model pages, 8 group pages, sitemap (144 urls)`. It also prints rolldown's "Some chunks are larger than 500 kB after minification" advisory, which is a suggestion about code-splitting and not an error; the exit code is 0
...
- [x] The sitemap dates are regenerated and committed — `npm run generate && git status --short src/sitemap-dates.json` prints nothing — run 2026-09-25 at `3c4100b`: prints nothing, so regenerating leaves the committed dates unchanged
