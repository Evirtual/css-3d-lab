...
| `signoff` | `scripts/signoff.mjs` | The two items on the release checklist that wait for a person: `npm run signoff` shows what is waiting, `-- article` and `-- push` tick them in `docs/RELEASE-CHECKLIST.md` with the date and commit, `-- undo <which>` takes one back. It refuses `push` while anything else that can be done before the push is still open. It pushes nothing and runs no check. |
