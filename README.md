...
| `queue` | `scripts/queue.mjs` | Records running and finished work in `docs/ledger-queue.json`: `npm run queue -- start "<name>" "<brief>" <model...>`, `-- progress "<name>" "<where it has got to>"`, `-- done "<name>"`, `-- list`. What it writes is the caller's own word, never a measurement, and the page labels it so — for what is measured, see `now` below. The record committed here is 94 jobs from the week this project's article describes; `echo '{"items":[]}' > docs/ledger-queue.json` starts a clean board, and nothing but that page reads the finished ones. |
